...
import { getDatabase, ref, push, set, onChildAdded, onValue, update, onChildChanged, onDisconnect, get, query, orderByChild, equalTo } from 'firebase/database';
...
let allUsers = {}; // Map of userId -> { userId, name, passcode, isAdmin }
...
function renderUsersList() {
  ...
  for (const userId in allUsers) {
    ...
    const isSelf = userId === currentUserUserId;
    
    const userItem = document.createElement('div');
    userItem.className = 'user-item';
    
    const editButton = `
      <button class="user-edit-btn" data-id="${userId}" title="Change Passcode">
        <i class="ph-bold ph-pencil"></i>
      </button>
    `;
    
    const deleteButton = isSelf ? '' : `
      <button class="user-delete-btn" data-id="${userId}" title="Delete User">
    ...
    userItem.querySelector('.user-edit-btn').addEventListener('click', async (e) => {
      e.stopPropagation();
      const newCode = prompt(`Enter new 6-digit passcode for "${user.name}":`);
      if (newCode !== null) {
...
function setupModalEvents() {
  ...
  document.getElementById('form-add-user').addEventListener('submit', async (e) => {
    ...
    const isPasscodeTaken = Object.values(allUsers).some(u => u.passcode === passcode);
    if (isPasscodeTaken) {
